...
export class Card2 {
  ...
  initEvents() {
    const mouseenterFn = (event) => this.enter(event);
    const mouseLeaveFn = (event) => this.leave(event);
    const focusFn = (event) => this.enter(event, true);
    const blurFn = (event) => this.leave(event, true);

    this.DOM.el.addEventListener("mouseenter", mouseenterFn);
    this.DOM.el.addEventListener("mouseleave", mouseLeaveFn);
    this.DOM.el.addEventListener("focus", focusFn);
    this.DOM.el.addEventListener("blur", blurFn);
  }
